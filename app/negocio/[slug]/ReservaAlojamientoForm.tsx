...
export default function ReservaAlojamientoForm({
  slug,
  habitaciones,
  colorPrincipal,
  tema = "oscuro",
  ...
  const [habitacionId, setHabitacionId] =
    useState(habitaciones[0]?.id || "");
  ...
  const totalEstimado =
    ...
    habitacionSeleccionada?.precio
      ? noches * habitacionSeleccionada.precio
      : 0;
  ...
  async function reservar(
    ...
    try {
      ...
      const resultado =
        (await respuesta.json()) as RespuestaReserva;
      ...
      setExito(
        `Solicitud enviada para ${
          ...
          "la habitación"
        } del ${resultado.fechaEntrada || fechaEntrada} al ${
          resultado.fechaSalida || fechaSalida
        }.`,
      );
      ...
      console.error(
        "Error enviando reserva de alojamiento:",
        errorReserva,
      );
      setError(
    ...
    >
      <div className="flex items-start gap-3 sm:gap-4">
        ...
        <div>
          ...
          >
            Elegí habitación, entrada, salida y
      ...
      >
        ...
        {(noches > 0 ||
          totalEstimado > 0) && (
          <div
            className={`flex flex-wrap items-center gap-x-5 gap-y-2 rounded-xl border p-3 text-xs sm:col-span-2 sm:p-4 sm:text-sm ${
              esClaro
          ...
          <div className="flex items-start gap-3 rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-3 text-xs text-emerald-300 sm:col-span-2 sm:p-4 sm:text-sm">
            ...
            <div>
              ...
              <p className="mt-1.5 text-[11px] text-emerald-400/80 sm:text-xs">
                El alojamiento podrá confirmar
                la solicitud desde su agenda.
              </p>
            </div>
          </div>
        )}
